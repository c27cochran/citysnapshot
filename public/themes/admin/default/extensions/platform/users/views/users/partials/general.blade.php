...
<div class="form-group{{ $errors->first('first_name', ' has-error') }}">
	<label for="first_name" class="col-lg-3 control-label">{{{ trans('platform/users::users/form.first_name') }}}</label>
	<div class="col-lg-9">
		<input type="text" class="form-control" name="first_name" id="first_name" placeholder="{{{ trans('platform/users::users/form.first_name') }}}" value="{{{ Input::old('first_name', ! empty($user) ? $user->first_name : null) }}}">
...
<div class="form-group{{ $errors->first('last_name', ' has-error') }}">
	<label for="last_name" class="col-lg-3 control-label">{{{ trans('platform/users::users/form.last_name') }}}</label>
	<div class="col-lg-9">
		<input type="text" class="form-control" name="last_name" id="last_name" placeholder="{{{ trans('platform/users::users/form.last_name') }}}" value="{{{ Input::old('last_name', ! empty($user) ? $user->last_name : null) }}}">
...
<div class="form-group{{ $errors->first('email', ' has-error') }}">
	...
	<div class="col-lg-9">
		<input type="text" class="form-control" name="email" id="email" placeholder="{{{ trans('platform/users::users/form.email') }}}" value="{{{ Input::old('email', ! empty($user) ? $user->email : null) }}}" required>
...
<div class="form-group{{ $errors->first('activated', ' has-error') }}">
	...
	<div class="col-lg-9">
		<select class="form-control" name="activated" id="activated" required>
			<option value="1"{{ Input::old('activated', $isActivated) == '1' ? ' selected="selected"' : null }}>{{ trans('general.yes') }}</option>
			<option value="0"{{ Input::old('activated', $isActivated) == '0' ? ' selected="selected"' : null }}>{{ trans('general.no') }}</option>
		</select>
...
<div class="form-group{{ $errors->first('password', ' has-error') }}">
	<label for="password" class="col-lg-3 control-label">{{{ trans("platform/users::users/form.{$mode}.password") }}}</label>
	<div class="col-lg-9">
